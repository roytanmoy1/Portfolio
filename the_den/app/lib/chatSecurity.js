...
const injectionPatterns = [
	/\b(?:ignore|disregard|override)\b.{0,50}\b(?:instruction|prompt|rule|policy)\b/i,
	/\b(?:system|developer|hidden)\s+(?:prompt|message|instruction|policy)\b/i,
	/\b(?:reveal|show|print|repeat|return|leak|extract)\b.{0,60}\b(?:prompt|instruction|secret|key|credential|environment|configuration)\b/i,
	/\b(?:api\s*key|access\s*token|password|credential|secret|process\.env|environment\s+variable)\b/i,
...
export function getGuardrailRefusal(message, { hasConversation = false } = {}) {
